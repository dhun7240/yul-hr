...
function isActive(pathname: string, href: string) {
  if (href === "/labor/dashboard") {
    return pathname === "/labor" || pathname === "/labor/dashboard";
  }

  return pathname === href || pathname.startsWith(`${href}/`);
}

type LaborSidebarProps = {
  mobile?: boolean;
};

export default function LaborSidebar({ mobile = false }: LaborSidebarProps) {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);

  const items = [
    { href: "/labor/dashboard", label: "대시보드" },
    { href: "/labor/companies", label: "자문사 관리" },
  ];
